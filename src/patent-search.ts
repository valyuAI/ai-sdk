 ...
 *
 * @param config - Configuration options for the Valyu patent search
 ...
 *   model: openai('gpt-5'),
 *   prompt: 'Find patents related to quantum computing algorithms',
 *   tools: {
 ...
 * ```
 */
export function patentSearch(config: ValyuPatentSearchConfig = {}) {
  const {
    ...
    execute: async ({ query }) => {
      ...
      try {
        ...
        if (error instanceof Error) {
          throw new Error(`Failed to search patents with Valyu: ${error.message}`);
        }
        throw error;
      }
    },
